  ...
  def main(args: Array[String]) {
    ...
    val requests = lines.map(x => {
      ...
      if (matcher.matches()) {
        ...
        (uuid,ip, url, matcher.group(6).toInt, matcher.group(9))
      } else {
        (randomUUID().toString,"error", "error", 0, "error")
      }
    })
    ...
    requests.foreachRDD((rdd, time) => {
      ...
      rdd.saveToCassandra("fotis", "logtest", SomeColumns("log_id","ip", "url", "status", "useragent"))
    })
